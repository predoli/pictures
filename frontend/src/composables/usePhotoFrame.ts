...
export function usePhotoFrame(preloadAheadCount: number = PRELOAD_AHEAD_COUNT) {
  ...
  const fetchImages = async (lastImage?: string): Promise<void> => {
    ...
    try {
      ...
      // If running from file:// protocol, try to fetch from static JSON first
      if (window.location.protocol === 'file:') {
        try {
          // Try to load static images.json file generated by backend
          response = await fetch('./images.json');
          if (!response.ok) {
            throw new Error('Static JSON not available, trying HTTP backend');
          }
        } catch (staticError) {
          console.warn('Static JSON failed, trying HTTP backend:', staticError);
          // Fallback to HTTP backend (requires --disable-web-security)
          ...
          response = await fetch(`${API_BASE}/images?${params}`);
        }
      } else {
        // Running from HTTP server, use normal API
        const params = new URLSearchParams({
          count: IMAGES_PER_BATCH.toString(),
          ordering: currentOrdering.value
        });
